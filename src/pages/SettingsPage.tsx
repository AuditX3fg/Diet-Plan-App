...
export function SettingsPage({ profile, language, theme, account, reminders, notificationPermission, onProfileChange, onLanguageChange, onThemeChange, onReminderChange, onEnableNotifications, onTestNotification, onOpenPlanImport, onLogout }: SettingsPageProps) {
  ...
  return (
    <div className="settings-page">
      <section className="feature-intro"><div><span className="status-pill"><Settings2 size={14} /> {tr('الإعدادات', 'Settings')}</span><h2>{tr('التطبيق كما تفضله', 'Make the app yours')}</h2><p>{tr('يتم حفظ كل تغيير تلقائياً ومزامنته بأمان مع حسابك.', 'Every change is saved automatically and securely synchronized with your account.')}</p></div><span className="saved-pill"><Check size={14} /> {tr('حفظ ومزامنة تلقائية', 'Autosave & sync')}</span></section>
      <section className="settings-grid">
        <div className="settings-card card-surface"><div className="settings-title"><span><UserRound size={19} /></span><div><small>{tr('الحساب', 'Account')}</small><h3>{tr('ملفك الشخصي', 'Your profile')}</h3></div></div><label className="setting-input"><span>{tr('اسم العرض', 'Display name')}</span><input value={language !== 'ar' && profile.name === 'محمد' ? 'Mohammed' : profile.name} onChange={(event) => onProfileChange({ ...profile, name: event.target.value })} placeholder={tr('اكتب اسمك', 'Enter your name')} /></label><label className="setting-input"><span>{tr('اسم المستخدم', 'Username')}</span><input value={`@${account.username}`} readOnly /></label>{account.email && <label className="setting-input"><span>{tr('بريد الاسترداد', 'Recovery email')}</span><input value={account.email} readOnly /></label>}</div>
