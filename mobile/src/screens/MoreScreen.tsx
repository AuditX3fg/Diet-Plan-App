...
import { cloudTrainingVideoSource, deleteCloudTrainingVideo, listCloudTrainingVideos, moveCloudTrainingVideo, submitCloudFeedback, updateCloudWorkoutPreference, uploadCloudTrainingVideo, type CloudTrainingVideo } from '../services/cloud'
...
function PersonalWorkoutsPanel({ data, theme, onUseDefault, onUseOwnLibrary }: { data: AppData; theme: AppTheme; onUseDefault: () => Promise<boolean>; onUseOwnLibrary: () => Promise<boolean> }) {
...
function WorkoutsPanel({ account, data, theme, onChangeAccount, onChangeData }: { account: UserAccount; data: AppData; theme: AppTheme; onChangeAccount: (next: UserAccount) => void; onChangeData: (next: AppData) => void }) {
...
function FeedbackPanel({ language, theme }: { language: Language; theme: AppTheme }) {
  const [rating, setRating] = useState(5)
  const [category, setCategory] = useState('experience')
  const [message, setMessage] = useState('')
  const [status, setStatus] = useState('')
  const [busy, setBusy] = useState(false)
  const categories = [{ id: 'experience', label: 'Experience' }, { id: 'bug', label: 'Bug' }, { id: 'feature', label: 'Feature' }, { id: 'meals', label: 'Meals' }, { id: 'workouts', label: 'Workouts' }, { id: 'general', label: 'General' }]

  async function send() {
    setBusy(true)
    setStatus('')
    try {
      const result = await submitCloudFeedback({ rating, category, message, page: 'settings', platform: Platform.OS === 'ios' || Platform.OS === 'android' ? Platform.OS : 'unknown' })
      setMessage('')
      setStatus(result.emailSent ? tr(language, 'Feedback sent successfully.', 'تم إرسال ملاحظاتك بنجاح.') : tr(language, 'Feedback saved and visible to the administrator.', 'تم حفظ ملاحظاتك وستظهر للمشرف.'))
    } catch (caught) { setStatus(caught instanceof Error ? caught.message : tr(language, 'Could not send feedback.', 'تعذر إرسال الملاحظات.')) }
    finally { setBusy(false) }
  }

  return <View style={styles.feedbackPanel}><SectionHeader eyebrow={tr(language, 'YOUR EXPERIENCE', 'تجربتك')} title={tr(language, 'Send feedback', 'أرسل ملاحظاتك')} caption={tr(language, 'Share an idea or report a problem. It is stored securely and shown to the administrator.', 'شارك اقتراحاً أو مشكلة. تُحفظ الرسالة بأمان وتظهر للمشرف.')} theme={theme} /><View style={styles.feedbackStars}>{[1, 2, 3, 4, 5].map((value) => <Pressable key={value} onPress={() => setRating(value)} style={[styles.feedbackStar, { borderColor: value <= rating ? '#d7aa44' : theme.line, backgroundColor: value <= rating ? '#fff5d8' : theme.surface }]}><Text style={{ color: value <= rating ? '#c58e1c' : theme.muted, fontSize: 20 }}>★</Text></Pressable>)}</View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.feedbackCategories}>{categories.map((item) => <Pressable key={item.id} onPress={() => setCategory(item.id)} style={[styles.feedbackCategory, { borderColor: category === item.id ? theme.primary : theme.line, backgroundColor: category === item.id ? theme.primarySoft : theme.surface }]}><Text style={{ color: category === item.id ? theme.primary : theme.muted, fontSize: 9, fontWeight: '800' }}>{item.label}</Text></Pressable>)}</ScrollView><TextInput multiline value={message} onChangeText={setMessage} maxLength={2000} placeholder={tr(language, 'Tell us what worked or what could be better…', 'اكتب ما أعجبك أو ما يمكن تحسينه…')} placeholderTextColor={theme.muted} style={[styles.feedbackInput, { color: theme.text, borderColor: theme.line, backgroundColor: theme.surface }]} /><PrimaryButton label={tr(language, 'Send feedback', 'إرسال الملاحظات')} onPress={() => void send()} busy={busy} disabled={message.trim().length < 10} theme={theme} />{status ? <Text style={[styles.feedbackStatus, { color: theme.primary }]}>{status}</Text> : null}</View>
}
