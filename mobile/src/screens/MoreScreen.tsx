...
function ScannerPanel({ data, theme, onChangeData }: { data: AppData; theme: AppTheme; onChangeData: (next: AppData) => void }) {
  ...
  async function lookup(code: string) {
    ...
    try {
      ...
      if (!json.product || json.status !== 1) throw new Error('Product not found. Try another barcode.')
      const product = json.product
      const nutrition = (product.nutriments ?? {}) as Record<string, number>
      const item: FoodProduct = {
        id: String(product._id ?? code), barcode: code,
        name: String(product.product_name_en ?? product.product_name ?? 'Scanned product'),
        ...
        serving: String(product.serving_size ?? 'per 100 g'),
        grade: typeof product.nutriscore_grade === 'string' ? product.nutriscore_grade.toUpperCase() : undefined,
      }
